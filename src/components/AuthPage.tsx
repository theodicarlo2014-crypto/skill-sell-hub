...
const AuthPage = () => {
  const { authMode, setAuthMode, userRole, setUserRole, setPage } = useAppStore();
    ...
    try {
      if (isLogin) {
        ...
        const { error } = await supabase.auth.signUp({
          ...
          password,
          options: {
            data: { full_name: name || 'User', role: userRole },
  ...
  const socialAuth = async (provider: 'google' | 'apple') => {
    ...
    try {
      ...
      // Session set — auth listener will handle the rest
      toast.success('Signed in successfully');
    } catch (err) {
    ...
    <div className="min-h-[calc(100vh-70px)] flex items-center justify-center px-4 py-8 relative">
      ...
      <div className="w-full max-w-[440px] bg-surface border border-border rounded-2xl p-8 md:p-10 relative">
        ...
        <div className="flex flex-col gap-3 mb-6">
          <button disabled={loading} onClick={() => socialAuth('google')} className="flex items-center justify-center gap-3 py-3 rounded-sm border border-border bg-surface2 text-foreground text-sm font-medium hover:bg-background hover:-translate-y-0.5 transition-all disabled:opacity-50">
          ...
          <button disabled={loading} onClick={() => socialAuth('apple')} className="flex items-center justify-center gap-3 py-3 rounded-sm border border-border bg-surface2 text-foreground text-sm font-medium hover:bg-background hover:-translate-y-0.5 transition-all disabled:opacity-50">
        ...
        <div className="flex flex-col gap-4">
          ...
          <button disabled={loading} onClick={handleSubmit} className="w-full py-3.5 bg-primary text-primary-foreground rounded-sm font-medium hover:bg-accent hover:-translate-y-0.5 hover:shadow-[0_6px_20px_hsl(var(--accent-glow))] transition-all mt-1 disabled:opacity-50">
